...
param conatinerAppsEnvName string = ''
...
module acr 'core/host/containerRegistry.bicep' = {
  ...
  params: {
    location: location
    containerAppName: !empty(backendServiceName) ? backendServiceName : '${abbrs.appContainerApps}backend-${resourceToken}'
...
module conatinerAppsEnv 'core/host/containerenv.bicep' = {
  name: 'conatinerAppsEnv'
  scope: resourceGroup
  params: {
    name: !empty(conatinerAppsEnvName) ? conatinerAppsEnvName : '${abbrs.appManagedEnvironments}${resourceToken}'
    location: location
    containerAppLogAnalyticsName:monitoring.outputs.logAnalyticsName

  }
}

module backend 'core/host/containerapps.bicep' = {
  name: 'web'
  scope: resourceGroup
  params: {
    name: !empty(backendServiceName) ? backendServiceName : '${abbrs.appContainerApps}backend-${resourceToken}'
    location: location
    managedIdentity: true
    containerAppEnvName:conatinerAppsEnv.outputs.containerAppEnvName
    containerRegistryName:acr.outputs.acrName
    acrHostedImage:acrImportImage.outputs.importedImages[0].acrHostedImage
    applicationinsightsConnectionString: useApplicationInsights ? monitoring.outputs.applicationInsightsConnectionString : ''
    azureStorageAccount: storage.outputs.name
    ...
    azureOpenaiGpt35TurboDeployment: openAiGpt35TurboDeploymentName
    azureOpenaiGpt35Turbo16kDeployment: openAiGpt35Turbo16kDeploymentName
